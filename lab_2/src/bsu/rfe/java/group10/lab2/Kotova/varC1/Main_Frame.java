package bsu.rfe.java.group10.lab2.Kotova.varC1;
...
public class Main_Frame extends JFrame{
    ...
    public Double formula1(Double x, Double y, Double z)
    {
        ...
        return Math.sin(Math.log(y) + Math.sin(Math.PI * y * y)) * Math.pow(x*x + Math.sin(z)
                + Math.exp(Math.cos(z)),1/4);
    }
    public Double formula2(Double x, Double y, Double z)
    {
        return Math.pow(Math.cos(Math.exp(x)) + Math.log(Math.pow((1+y),2))  + Math.pow((Math.exp(Math.cos(x))
                + Math.pow(Math.sin(Math.PI * z),2)),1/2) + Math.pow(1/x,1/2) + Math.cos(y*y),Math.sin(z));
    }
    ...
    private void addRadioButton(String name, final int formula_number)           // радиокнопки для формул
    {
        ...
        button.addActionListener(new ActionListener()
        {
            ...
            public void actionPerformed(ActionEvent actionEvent)
            {
                ...
                if (formula_number == 1)	image.setIcon(new ImageIcon(Main_Frame.class.getResource("function1.jpg")));
                if (formula_number == 2)    image.setIcon(new ImageIcon(Main_Frame.class.getResource("function2.jpg")));
            }
        });
    ...
    public Main_Frame() {
        ...
        image = new JLabel(new ImageIcon(Main_Frame.class.getResource("function1.jpg")));
        picture.add(image);
        picture.add(Box.createHorizontalGlue());
        ...
        data.add(x_field);
        data.add(Box.createHorizontalGlue());
        data.add(Box.createHorizontalStrut(120));
        data.add(y_label);
        data.add(Box.createHorizontalStrut(10));
        data.add(y_field);
        data.add(Box.createHorizontalGlue());
        data.add(Box.createHorizontalStrut(200));
        data.add(z_label);
        data.add(Box.createHorizontalStrut(10));
        data.add(z_field);
        data.add(Box.createHorizontalGlue());
